...
const VueRouter = (function(exports, vue) {
  ...
  function parseURL(parseQuery, location, currentLocation = '/') {
    let path; let query = {}; let searchString = ''; let
      hash = '';
    ...
    for (const segment of segments) {
      ...
      for (let tokenIndex = 0; tokenIndex < segment.length; tokenIndex++) {
        ...
        if (token.type === 0 /* Static */) {
          ...
          const {
            value, repeatable, optional, regexp,
          } = token;
          keys.push({
            name: value,
            repeatable,
    ...
    function insertMatcher(matcher) {
      ...
      while (i < matchers.length &&
              ...
              (matcher.record.path !== matchers[i].record.path ||
                  !isRecordChildOf(matcher, matchers[i]))) i++;
      ...
      const next = (valid) => {
        if (valid === false) {
          ...
          if (enterCallbackArray &&
                      ...
                      typeof valid === 'function') enterCallbackArray.push(valid);
          resolve();
        }
      };
    ...
    for (const key in inner) {
      ...
      if (typeof innerValue === 'string') {
        if (innerValue !== outerValue) return false;
                  ...
                  innerValue.some((value, i) => value !== outerValue[i])) return false;
    }
    return true;
  }
  /**
  ...
  function isRouteMatching(route, filter) {
    ...
    if (!filter.startsWith('/') &&
          (decodedPath.includes(filter) || path.includes(filter))) return true;
