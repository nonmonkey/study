var Vue = (function (exports) {
  ...
  let uid = 0;
  // 该函数 的作用主要是配置了一些初始化的参数，然后包装了之前传进来的fn，重要的一点是吧当前的effect赋值给了activeEffect，这一点非常重要，和收集依赖有着直接的关系。
  function createReactiveEffect(fn, options) {
      const effect = function reactiveEffect() {
          ...
          if (!effectStack.includes(effect)) {
              ...
              try {
                  enableTracking();
                  effectStack.push(effect); // 往effect数组中放入当前的effect
                  activeEffect = effect; // effect 赋值给当前的activeEffect
                  console.log('createReactiveEffect', 'try')
                  return fn();
              }
              finally {
                  effectStack.pop(); // 完成依赖后从effect数组删除这个effect
                  resetTracking();
                  activeEffect = effectStack[effectStack.length - 1]; // 还原到以前的effect
                  console.log('createReactiveEffect', 'finally')
              }
          }
      };
      effect.id = uid++;
  ...
  */
  ...
  function baseCreateRenderer(options, createHydrationFns) {
      ...
      const mountComponent = (initialVNode, container, anchor, parentComponent, parentSuspense, isSVG, optimized) => {
          ...
          // 2.初始化组件，建立proxy，根据字符串模板得到render函数，生命周期钩子函数等等
      ...
      // 1. 创建一个effect，并把它赋值给组件实例update方法，作为渲染更新视图用
      // 2. componentEffect 作为回调函数形式传递给effect作为第一个参数
